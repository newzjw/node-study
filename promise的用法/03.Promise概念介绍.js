// 1. Promise 是一个 构造函数，既然是构造函数， 那么，我们就可以new Promise() 得到一个 Promise 的实例；
// 2. 在 Promise 上，有两个函数，分别叫做 resolve（成功之后的回调函数） 和 reject（失败之后的回调函数）
// 3. 在 Promise构造函数的Prototype 属性上，有一个.then()方法，也就说，只要是Promise构造函数创建的实例，都可以访问到.then()方法
...
//  这时候，只能使用回调函数的形式，来把 成功 或 失败的结果，返回给调用者；
// 6. 我们可以在 new 出来的 Promise 实例上，调用 .then() 方法，【预先】 为 这个 Promise 异步操作，
...
// 也就是说，new的时候，除了能够得到一个promise实例之外，还会立即调用我们为Promise构造函数传递的那个function，
// 执行这个 function 中的 异步操作代码；
var promise = new Promise(function () {
...
// 封装成函数
// 初衷： 给路径，返回读取到的内容
// 返回一个promise对象，该对象可以调用then方法，调用 .then() 方法，【预先】 为 这个 Promise 异步操作，
// 指定 成功（resolve） 和 失败（reject） 回调函数
